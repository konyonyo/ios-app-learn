...
- メッセージを端末内へ保存
- アプリ再起動後も会話データを復元
- セッションをスワイプして削除（確認ダイアログ付き）
- 新規セッションの最初のユーザーメッセージからタイトルを自動設定

セッションタイトルは追加のLLM呼び出しを行わず、最初のユーザーメッセージを空白・改行を整理して最大32文字に切り詰めたものを使用します。タイトル設定はユーザーメッセージを保存する直前に行うため、固定の初期アシスタントメッセージは作成されません。
